...
- Specifically, the first principal component ```v1``` is the direction that maximizes the variance along that direction in the projected data, namely, PC1 = ```v1'X``` has the maximum variance (which is actually ```λ1```) (and iteratively, PC2 = ```v2'X``` has the maximum variance while ```v2``` accounts for the remaining variation)
