...
To <a href="https://stats.stackexchange.com/questions/2691/making-sense-of-principal-component-analysis-eigenvectors-eigenvalues">summarize</a> ```X``` of ```p``` variables with fewer variables by finding ```L``` variables (principal components) that (a) show as much variation across data in ```X``` as possible and (b) allow to reconstruct the original ```p``` variables as well as possible, where ```L``` << ```p```
