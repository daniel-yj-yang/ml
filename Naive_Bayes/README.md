...
p(y|X) &Proportional; p(X|y) * p(y) = p(y) * p(x<sub>1</sub>|y) * p(x<sub>2</sub>|y) * ... * p(x<sub>n</sub>|y) = p(y) * product(i=1 to n)p(x<sub>i</sub>|y)

Then, based on the maximum value of p(y|X) across all values of y, we will classify the outcome to that specific value of y
